...
const docs: { [key: string]: ConfigDoc } = {
  ...
  'train.audio_loss_multiplier': {
    ...
    description: (
      <>
        控制音频损失在总训练损失中的权重。默认值 1.0 表示按原始权重加入；设为 0 会忽略音频损失；数值越大，
        训练越侧重音频，但过高可能影响画面学习或导致训练不稳定。
      </>
    ),
  },
  ...
  'datasets.batch_size': {
    ...
    description: (
      <>
        单独设置当前数据集每批读取的样本数量，用于覆盖训练设置中的全局批次大小。留空时使用全局值。
        此功能要求当前数据集启用分桶；使用自动帧数时必须设为 1。
      </>
    ),
  },
  ...
  'datasets.caption_dropout_rate': {
    ...
    description: (
      <>
        打标丢弃率表示每个训练步骤中，图片打标被丢弃（替换为空打标）的概率。例如设为 0.05，约有 5% 的训练步骤会丢弃打标。
        这样可以让模型学习训练概念时不过度依赖打标，并保留无提示词生成能力。设置了触发词时，丢弃打标后仍会保留触发词，
        使模型继续将样本与触发词关联。正则化图片或未设置触发词的图片会使用完全空白的打标。
        <br />
        <br />
        缓存文本嵌入时也支持打标丢弃。系统会在正常嵌入之外，将丢弃后的打标嵌入（空白或仅触发词）一并缓存到磁盘，
        并在训练时按该概率随机替换。
      </>
    ),
  },

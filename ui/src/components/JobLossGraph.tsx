...
export default function JobLossGraph({ job }: Props) {
  ...
  const handleDeleteSelectedRange = useCallback(() => {
    ...
    openConfirm({
      title: '删除选中范围',
      message: `确定要从所有指标的损失日志中永久删除第 ${minStep.toLocaleString()}–${maxStep.toLocaleString()} 步（图中 ${count.toLocaleString()} 个数据点）吗？此操作无法撤销。`,
      type: 'danger',
      confirmText: '删除',
      onConfirm: async () => {
        setDeleting(true);
      ...
      <div className="px-4 pb-2 shrink-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="bg-gray-950 border border-gray-800 rounded-lg p-3">
            <label className="block text-xs text-gray-400 mb-2">显示设置</label>
            <div className="flex flex-wrap gap-2">
              <ToggleButton checked={showTrend} onClick={() => setShowTrend(v => !v)} label="趋势线" />
              <ToggleButton checked={useLogScale} onClick={() => setUseLogScale(v => !v)} label="对数坐标" />
              <ToggleButton checked={clipOutliers} onClick={() => setClipOutliers(v => !v)} label="去除异常值" />
            </div>
          </div>

          <div className="bg-gray-950 border border-gray-800 rounded-lg p-3">
            <label className="block text-xs text-gray-400 mb-2">数据系列</label>
            {lossKeys.length === 0 ? (
              <div className="text-sm text-gray-400">暂无损失数据系列。</div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {lossKeys.map(k => (
                  <button
          ...
          <div className="bg-gray-950 border border-gray-800 rounded-lg p-3">
            ...
            <input
              ...
              className="w-full accent-blue-500"
            />
            <div className="mt-2 text-[11px] text-gray-500">UI 降采样，用于优化大数据量显示。</div>
          </div>
        </div>
      </div>

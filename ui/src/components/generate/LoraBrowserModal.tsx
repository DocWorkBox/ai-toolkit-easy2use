      ...
      <div className="text-gray-200">
        ...
        <div className="max-h-[60vh] overflow-y-auto pr-1">
          ...
          {tab === 'cloud' && (
            <div>
              <div className="text-[11px] text-gray-500 mb-2 flex items-center gap-1">
                <Cloud className="w-3.5 h-3.5" /> 首次使用时下载到 loras 目录
              </div>
              {filteredCloud.length === 0 && (
                <div className="text-xs text-gray-500">此模型暂无已发布的 LoRA。</div>
              )}
          ...
          {!loading && tab === 'jobs' && (
            <div className="space-y-1">
              {filteredJobs.length === 0 && (
                <div className="text-xs text-gray-500">没有保存过 LoRA 的训练任务。</div>
              )}
              {filteredJobs.map(j => {
          ...
          {tab === 'upload' && (
            <div>
              <div className="text-[11px] text-gray-500 mb-2 flex items-center gap-1">
                <FolderOpen className="w-3.5 h-3.5" /> 上传到 {lorasRoot || 'MODELS_PATH/loras'}
              </div>
              <input
                ref={fileInputRef}
                type="file"
              ...
              {!upload || upload.status === 'cancelled' || upload.status === 'error' ? (
                <div>
                  ...
                  >
                    ...
                    <span>点击选择 .safetensors 文件，或将文件拖到这里</span>
                    <span className="text-[11px] text-gray-500">
                      大文件将按 {fmtSize(CHUNK_SIZE)} 分块上传
                    </span>
                  </button>
                  ...
                  {upload?.status === 'cancelled' && (
                    <div className="mt-2 text-xs text-gray-500">已取消上传 {upload.file.name}。</div>
                  )}
                </div>
              ) : (
                <div className="rounded-md bg-gray-900 border border-gray-800 p-3">
                  ...
                  <div className="mt-1 flex items-center justify-between text-[11px] text-gray-500">
                    ...
                    {upload.status === 'uploading' && (
                      <span>
                        {fmtSize(rate)}/s{eta > 0 ? ` · 约剩 ${Math.ceil(eta)} 秒` : ''}
                      </span>
                    )}
                  </div>
                  {upload.status === 'uploading' && (
                  ...
                  {upload.status === 'done' && upload.result && (
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-xs text-green-400">上传完成。</span>
          ...
          {!loading && tab === 'models' && (
            <div>
              ...
              {filteredModels.length === 0 && (
                <div className="text-xs text-gray-500">loras 目录下没有找到 .safetensors 文件。</div>
              )}
              <div className="space-y-0.5">

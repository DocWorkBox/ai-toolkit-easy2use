      ...
      {showLog ? (
        <div>
          <div className="mb-2 text-sm">
            {isRunning && <span className="text-amber-400">正在融合 LoRA，请不要关闭此窗口。</span>}
            {isDone && hasError && <span className="text-rose-400">融合失败，请查看下方日志。</span>}
            {isDone && !hasError && <span className="text-emerald-400">融合完成。</span>}
          </div>
          <div
            ref={logRef}
        ...
        >
          ...
          {selectedLoRAs.length > 0 && (
            <div className="mt-4">
              <label className="block text-xs mb-1 text-gray-300">已选择的 LoRA</label>

    ...
    >
      <div>
        <div className="mb-3 flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
          ...
          <span>
            此操作会加载模型并占用 GPU 运行。建议至少预留 13GB 空闲显存；开始前请确认 GPU 空闲（没有训练或其他任务正在运行），否则可能显存不足。
          </span>
        </div>

        <div className="mb-2 flex items-center justify-between text-sm">
          <div className="text-gray-400">
            {isRunning && <span className="text-amber-400">正在扩写，请不要关闭此窗口。</span>}
            {isDone && hasError && <span className="text-rose-400">已完成，但存在错误。请查看下方日志。</span>}
            {isDone && !hasError && <span className="text-emerald-400">提示词扩写完成。</span>}
            {!isRunning && !isDone && (
              <span>
                选择要扩写为 Ideogram 结构化 caption 的提示词。已选择 {selectedCount}/{prompts.length} 条。
              </span>
            )}
          </div>
          {!isRunning && !isDone && (
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className="text-[11px] text-gray-500">
            {creative
              ? '会扩展原始想法，把主体放入合适场景并补充细节。'
              : '忠实模式：按原提示词整理结构，只补充最少背景。'}
          </span>
        </div>
        ...
        <div className="mb-3">
          ...
          <textarea
            ...
            disabled={isRunning || isDone}
            rows={2}
            placeholder="例如：保持为近景人像，优先白天场景，始终使用 9:16 竖构图..."
              ...
              <div key={p.index} className={classNames('flex items-start gap-2 px-3 py-2', { 'opacity-40': dim })}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {upsampled[p.index] && (
                      <span className="text-[10px] text-emerald-400 flex-shrink-0">已扩写</span>
                    )}
                  </div>
                  <div
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          {!isDone && (
            ...
            >
              {isRunning ? '正在扩写...' : `扩写 (${selectedCount})`}
            </button>
          )}
        </div>
      </div>

...
function GeneratePageInner() {
  ...
  const footerStatus = running
    ...
    : error
      ...
      : ready
        ? health?.current
          ? `正在生成 ${health.current.arch}`
          : health?.active?.arch
            ? `空闲 - ${health.active.arch} (${health.active.model?.name_or_path || ''})`
            : '空闲 - 未加载模型'
        : engineStatus?.engine
          ? `引擎状态：${engineStatus.engine.status}`
          : '引擎未启动';
    ...
    <>
      <TopBar>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base sm:text-lg">生成</h1>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs sm:text-sm">
          {isStopping ? (
            <>
              ...
              >
                <Loader2 className="w-4 h-4 animate-spin" /> 正在停止引擎…
              </Button>
              {stoppingForMs > 8000 && (
                <Button
                  onClick={forceStop}
                  ...
                  title="强制终止引擎进程"
                >
                  <OctagonX className="w-4 h-4" /> 强制停止
                </Button>
              )}
            </>
          ) : ready ? (
            <>
              <span className="text-green-400">引擎运行中</span>
              ...
              >
                {engineBusy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Square className="w-4 h-4" />} 停止
              </Button>
            </>
          ) : null}
        </div>
        ...
        {!ready && (
          <div className="absolute inset-0 z-20 flex items-center justify-center p-4">
            <div className="bg-gray-900/95 backdrop-blur border border-gray-700 rounded-2xl shadow-2xl px-8 py-8 max-w-md w-full text-center">
              ...
              <h2 className="text-lg text-gray-100 font-semibold mb-2">推理引擎</h2>
              <p className="text-sm text-gray-400 mb-6">
                {isStarting
                  ? '引擎正在启动，服务就绪后会显示模型选项。'
                  : '在 GPU 上启动推理引擎，即可使用已支持的模型生成图片、视频和音频。'}
              </p>
              {isStarting ? (
                <div className="flex flex-col gap-2">
                  ...
                  {engineStatus?.engine?.status === 'queued' && (
                    <div className="text-[11px] text-gray-500">
                      正在等待 GPU 队列接收任务。如果队列已停止，请在训练队列页面启动队列，或在此取消。
                    </div>
                  )}
            ...
            <div className="flex-1 min-h-0 relative bg-black/40 m-2 mb-0 rounded-xl border border-gray-800 overflow-hidden">
              ...
              <div className="absolute top-0 left-0 right-0 z-10 px-3 py-1.5 flex items-center gap-2 text-xs text-gray-300 bg-gradient-to-b from-gray-950/80 to-transparent">
                ...
                {progress ? (
                  <span className="text-gray-400 shrink-0">
                    步骤 {progress.step}
                    {progress.total ? `/${progress.total}` : ''} · {progress.elapsed.toFixed(1)} 秒
                  </span>
                ) : null}
                <div className="flex-1" />
                {running && (
              ...
              <div className={`absolute inset-3 flex items-center justify-center ${showPreview ? '' : 'hidden'}`}>
                ...
                {!running && framesRef.current.length === 0 && (
                  <div className="absolute text-gray-600 text-sm">生成结果会显示在这里</div>
                )}
              </div>
            ...
            <div className="h-20 shrink-0 m-2 flex items-center gap-2">
              <div className="flex-1 min-w-0 h-full flex items-center gap-2 overflow-x-auto overflow-y-hidden">
                ...
                {results.length === 0 && !running ? (
                  <div className="text-[11px] text-gray-600 px-1">历史记录</div>
                ) : (
                  results.map(r => {
          ...
          <div className={`relative shrink-0 transition-[width] duration-200 ${sidebarOpen ? 'w-[340px]' : 'w-0'}`}>
            ...
            >
              <div className="compact-form flex-1 min-h-0 overflow-y-auto p-2 space-y-2">
                ...
                >
                  ...
                  <CreatableSelectInput
                    ...
                    placeholder="模型仓库、本地目录或 .safetensors 文件"
                  />
                ...
                <Card
                  title="LoRA"
                  subtitle={loras.length ? `${activeLoras.length}/${loras.length} 个启用` : '无'}
                  open={cardOpen('loras')}
                ...
                >
                  <div className="space-y-2">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            title={l.disabled ? '已跳过：点击后用于推理' : '已启用：点击后跳过'}
                          >
                            <span
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <Plus className="w-3.5 h-3.5" /> 添加 LoRA
                      </Button>
                    </div>
                    <Checkbox
                      label="合并到权重"
                      ...
                      doc={{
                        title: '将 LoRA 合并到模型权重',
                        description: (
                          <>
                            <p>
                              <strong>关闭</strong>（默认）：每个 LoRA 以附加项动态作用于目标层。
                              强度修改会在下一次生成时生效，添加或移除 LoRA 不会重新加载模型。
                            </p>
                            <p className="mt-2">
                              <strong>开启</strong>：LoRA 差值会直接加入模型权重。量化权重会先反量化，
                              合并后使用随机舍入重新量化，避免较小差值被舍弃。合并后无法原地撤销，
                              修改 LoRA 组合或强度会重新加载相关组件；每一步会比动态方式略快。
                            </p>
                          </>
                        ),
                      }}
                ...
                <Card title="提示词" open={cardOpen('prompt')} onToggle={() => toggleCard('prompt')}>
                  {entry?.sampleTags ? (
                    ...
                    <TextAreaInput
                      ...
                      placeholder={
                        modality === 'audio' ? '歌曲描述、歌词、BPM…' : '描述要生成的内容'
                      }
                    />
                  )}
              ...
              <div className="shrink-0 p-2 border-t border-gray-800 bg-gray-900 flex gap-2">
                ...
                >
                  {running ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} 生成
                </Button>
                {running && (

...
def _healthy_torch_stack(stack):
    return bool(stack) and all(
        stack.get(name) and not stack[name].startswith("ERROR")
        for name in ("torch", "torchvision", "torchaudio")
    )


def _write_torch_constraints(lines, description, dry_run=False):
    path = os.path.join(venv_dir(), CONSTRAINTS_FILE)
    if not dry_run:
        with open(path, "w") as f:
            f.write("# Generated by the AI Toolkit manager (manager/env.py).\n")
            f.write("# %s\n" % description)
            f.write("# Do not edit.\n")
            f.write("\n".join(lines) + "\n")
...
def _torch_pin_args(spec, dry_run=False):
    ...
    args = _write_torch_constraints(
        spec.torch_constraints(),
        "Keeps requirements and accelerator wheels on the selected PyTorch build.",
        dry_run=dry_run,
    )
...
def _torch_preservation_args(stack, dry_run=False):
    """Constraints that keep targeted repairs on the already-working stack."""
    if not _healthy_torch_stack(stack):
        return []
    lines = [
        "%s==%s" % (name, stack[name])
        for name in ("torch", "torchvision", "torchaudio")
    ]
    return _write_torch_constraints(
        lines,
        "Preserves the working PyTorch stack during targeted dependency repair.",
        dry_run=dry_run,
    )


def _verify_torch_preserved(original_stack, dry_run=False):
    """Fail safely if a targeted repair changed the existing PyTorch stack."""
    if dry_run or not _healthy_torch_stack(original_stack):
        return False
    current_stack = torch_stack()
    ...
    die(
        "Targeted dependency repair changed the PyTorch stack "
        "(%s -> %s). Automatic PyTorch replacement is disabled; stopping "
        "without starting another download."
        % (describe(original_stack), describe(current_stack))
    )
...
def repair_requirements(spec, dry_run=False):
    ...
    original_torch = torch_stack()
    preserve_torch = _healthy_torch_stack(original_torch)
    pins = (
        _torch_preservation_args(original_torch, dry_run=dry_run)
        if preserve_torch
        else _torch_pin_args(spec, dry_run=dry_run)
    )
    ...
    if preserve_torch:
        _verify_torch_preserved(original_torch, dry_run=dry_run)
    else:

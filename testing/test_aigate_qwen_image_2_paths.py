...
LOCAL_CONFIG_DIR = "/datasets/studio/huggingface/models/Qwen-Image-2.1"
...
def test_qwen_image_2_uses_local_configs_with_comfy_weights():
    ...
    assert f'BASE_REPO = "{LOCAL_CONFIG_DIR}"' in model_source
    ...
    for path in (
        ...
        assert f'aitk_config_repo = "{LOCAL_CONFIG_DIR}"' in _source(path)

    text_encoder_source = _source(
        "extensions_built_in/diffusion_models/qwen_image_2/src/text_encoder.py"
    )
    assert f'aitk_processor_repo = "{LOCAL_CONFIG_DIR}"' in text_encoder_source

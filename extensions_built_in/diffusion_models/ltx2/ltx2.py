...
class LTX25Model(LTX2Model):
    ...
    def _resolve_named_file(self, path: str, component: str) -> str:
        ...
        local_path = get_path(path)
        if os.path.exists(local_path):
            return local_path

        normalized_path = path.replace("\\", "/")
        portable_prefix = "./models/"
        if normalized_path.startswith(portable_prefix):
            rel_path = normalized_path[len(portable_prefix) :]
            for candidate in (
                os.path.join(MODELS_PATH, rel_path),
                os.path.join(MODELS_PATH, os.path.basename(rel_path)),
            ):
                if os.path.exists(candidate):
                    return candidate
            raise FileNotFoundError(
                f"LTX-2.5 {component} not found under MODELS_PATH: {rel_path}. "

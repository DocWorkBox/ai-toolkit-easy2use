...
base_te_path = "/model/ModelScope/Lightricks/gemma-3-12b-it-qat-q4_0-unquantized"
...
class LTX25Model(LTX2Model):
    ...
    def _resolve_comfy_file(self, component: str) -> str:
        ...
        Search order: model_kwargs override, a local name_or_path directory,
        MODELS_PATH (diffusion_models/, text_encoders/, vae/), then the hub.
        Local roots accept the repo-relative path, the bare filename, or a
        matching filename below the component's category folder.
        """
        ...
        if name_or_path and os.path.isdir(name_or_path):
            search_roots.append(name_or_path)
        if MODELS_PATH not in search_roots:
            search_roots.append(MODELS_PATH)

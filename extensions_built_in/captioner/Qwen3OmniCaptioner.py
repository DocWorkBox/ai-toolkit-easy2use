...
class Qwen3OmniCaptioner(BaseCaptioner):
    ...
    def _resolve_checkpoint(self) -> str:
        ...
        from toolkit.paths import MODELS_PATH, get_path
        ...
        local_name_or_path = get_path(name_or_path)
        if name_or_path.lower().endswith(".safetensors"):
            self._model_info = info_for_filename(os.path.basename(name_or_path))
            if os.path.isfile(local_name_or_path):
                return local_name_or_path
            raise FileNotFoundError(
                f"Qwen3-Omni checkpoint not found: {local_name_or_path}. "
                "Download it from Model Management and place it at this path."
            )
        ...
        if os.path.isdir(local_name_or_path):
            candidate = os.path.join(local_name_or_path, filename)
            if os.path.exists(candidate):
                self._model_info = model_info
                return candidate
            files = [
                f for f in os.listdir(local_name_or_path) if f.endswith(".safetensors")
            ]
            if len(files) == 1:
                self._model_info = info_for_filename(files[0])
                return os.path.join(local_name_or_path, files[0])
            raise FileNotFoundError(
                f"No {filename} (or single .safetensors) in {local_name_or_path}"
            )
    ...
    def _resolve_base_repo(self) -> str:
        from toolkit.paths import get_path

        base_repo = get_path(self._model_info["base_repo"])
        missing = [
            filename
            for filename in BASE_METADATA_FILES
            if not os.path.isfile(os.path.join(base_repo, filename))
        ]
        if missing:
            raise FileNotFoundError(
                f"Qwen3-Omni processor metadata is incomplete in {base_repo}. "
                f"Missing: {', '.join(missing)}. Download the metadata from "
                "Model Management and place it in this directory."
            )
        return base_repo
    ...
    def load_model(self):
        ...
        config = AutoConfig.from_pretrained(base_repo, local_files_only=True)
